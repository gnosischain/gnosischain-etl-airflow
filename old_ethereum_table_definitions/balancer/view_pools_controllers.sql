...
with events as (
    ...
    to_address as pool, manager
    from `blockchain-etl.ethereum_balancer.BPool_call_setController`
    where status=1
),
...
state_with_gaps as (
    select events.block_number, events.pool, events.manager,
...
calendar AS (
    select number as block_number from `bigquery-public-data.crypto_gnosischain.blocks`
),
running_state as (
    select pool, calendar.block_number, manager
